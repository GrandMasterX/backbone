$(document).ready(function() {
    ...
    logSteps( window.introVideo, 'Стартовая страница регистрации с видео intro', 1);
});
            ...
            var vimeoPlayers = $('#'+id),
                ...
                function addEvent(element, eventName, callback) {
                    ...
                    function onPlay(player_id) {
                        $(".start-btn-holder").fadeOut();   
                        logSteps(id, 'Запуск видео', 2);
                        console.log('start');
                    }
                    
                    function onPause(player_id) {
                        $(".start-btn-holder").fadeIn('slow');
                        logSteps(id, 'Остановка видео', 3);
                        console.log('paused');
                    }

                    function onFinish(player_id) {
                        $(".start-btn-holder").fadeIn('slow');
                        logSteps(id, 'Завершение видео', 5);
...
//        }
    ...
    $(':submit').live('click', function() {
            logSteps( null, 'Просмотр видео завершен, начинаем регистрацию', 7);
    });         
    ...
    $('.next-btn').live('click', function() {
        if ($(this).hasClass('first')) {
            ...
            logSteps( null, 'Продолжаем снятие размеров', 6);
    ...
    function loadVideo(elem) {
        ...
        init(vidId);
        if(vidId!=undefined) {
            logSteps( vidId, 'Просмотр видео', 4);
        }
    }
    
    //log every step 
    function logSteps(video_id, message, stage) {
        jQuery.ajax({
            'url':'logSteps',
            'dataType':'json',
            'type':'GET',
