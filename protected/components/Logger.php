 ...
 * Logger class file
 *
 * @author TAE <moneystream@mail.ru>
  * @copyright Copyright &copy; 2013 Yii Software LLC
 ...
 *
 * Logger implements the methods to retrieve the messages with
 * various filter conditions, including log levels and log categories.
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @version $Id: Logger.php 01 2013-08-12$
...
class Logger extends CLogger
{
     ...
     * @see getLogs
     */
    public function log($message,$level='info',$category='application', $data=array())
    {                  
        $this->_logs[] = array_merge( array($message,$level,$category,microtime(true)), $data);
        $this->_logCount++;
    ...
    public function getLogs($levels='',$categories='')
    {
        ...
        //echo "<pre><br>";   print_r($categories);    die;
        ...
        if (is_array($categories)) {$categories='';} else {    ////echo "categories = ." .print_r($categories, true);  die;
...
/*    public function collectLogs() {
        parent::collectLogs()
        echo "<br>collectLogs";    die;         //////// не вызывается судя по всему
    }*/
