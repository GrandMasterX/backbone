	...
	protected function oAuth( $provider )
	{
		try{
			...
			if(!Yii::app()->user->isGuest)
			{
                ...
                User::setSocialData($oAuth->profile,Yii::app()->user->id);
			}
			else 
			{
                $newUser = false;
				...
				if(!$oAuth->isBond || !$user)
				{
					...
					if(!isset($user))
					{
                        ...
                        //if($provider =='Facebook' || $provider =='Vkontakte')$this->scenario = 'insertSocial';
                        $user = new $this->model();
						$newUser = true;
					}
				...
				if($accessCode === 1)
				{
                    Yii::import(self::ALIAS . '.models.UserIdentity');
	...
	protected function processUser($user, $userProfile)
	{
		...
		if($user->isNewRecord)
		{
                    if(!$user->save()) {
                        ...
                        User::setSocialData($userProfile,$user->id);
                    }
