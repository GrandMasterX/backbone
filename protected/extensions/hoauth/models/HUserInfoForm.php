...
class HUserInfoForm extends CFormModel {
	...
	{
		return array(
			'email'=>Yii::t('oauth','Email'),//HOAuthAction::t('Email'),
			'username'=>Yii::t('oauth','Имя пользователя'),//HOAuthAction::t('Nickname'),
			'password'=>Yii::t('oauth','Пароль'),//HOAuthAction::t('Password'),
			);
	}
	...
	{
		return Yii::t('oauth',"Этот {attribute} уже используется в учетной записи. Если она принадлежит вам, введите ее пароль или измените {attribute}, а поле с паролем оставьте пустым.", array('{attribute}'=>$this->getAttributeLabel($attributeName)));
			...
			$this->_form = new CForm(array(
				...
				'buttons'=>array(
					'submit'=>array(
						...
						'label'=>Yii::t('oauth','Дальше'),//HOAuthAction::t('Submit'),
						),
					'</div>',
					),
				'activeForm'=>array(
	...
	public function getHeader()
	{
		...
		{
			case 'both':
			$header = Yii::t('oauth','Пожалуйста, укажите ваши имя пользователя и email.');//HOAuthAction::t('Please specify your nickname and email to end with registration.');
			break;
			case 'username':
			$header = Yii::t('oauth','Пожалуйста, укажите ваше имя пользователя.');//HOAuthAction::t('Please specify your nickname to end with registration.');
			break;
			case 'email':
			$header = Yii::t('oauth','Пожалуйста, укажите ваш email.');//HOAuthAction::t('Please specify your email to end with registration.');
			break;
		}
